...
namespace HTQuanLyXe.DAO
{
    class baixeDAO
    {
        ...
        public int tongxedaptrong()
        {
            int so = 0;
            String Query = "select * from BAIXE where loaiXe=N'Xe đạp'";
            DataTable data = new DataTable();
            data = dataprovider.Instance.ExcuteQuery(Query);
            foreach (DataRow item in data.Rows)
            {
        ...
        public Boolean thaydoiben(int oto, int xemay, int xedap)
        {
            ...
            string query6 = "update BAIXE set soChoTrong=(" +xedap + "-(select count(bienSo) from XE where loaiXe='Xe đạp')) where loaiXe=' Xe đạp'";
            ...
            string query5 = "update BAIXE set tongSoCho=" + xedap + " where loaiXe='Xe đạp'";
            ...
            int c = dataprovider.Instance.ExcuteNonQuery(query5);
            int g = dataprovider.Instance.ExcuteNonQuery(query6);

            return (0<m && 0<o && 0 < g && 0<a && 0<b && 0<c);
        }
        //tiền vé xe máy
        ...
        public int tienvexedap()
        {
            int gia = 0;
            String Query = "select * from BAIXE where loaiXe='Xe đạp'";
            DataTable data = new DataTable();
            data = dataprovider.Instance.ExcuteQuery(Query);
            foreach (DataRow item in data.Rows)
            {
